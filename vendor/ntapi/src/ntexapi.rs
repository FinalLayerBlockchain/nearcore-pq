...
pub unsafe fn NtGetTickCount64() -> ULONGLONG {
    ...
    #[cfg(target_arch = "x86")] {
        loop {
            tick_count.s_mut().HighPart =
                read_volatile(core::ptr::addr_of!((*USER_SHARED_DATA).u.TickCount.High1Time)) as u32;
            tick_count.s_mut().LowPart = read_volatile(core::ptr::addr_of!((*USER_SHARED_DATA).u.TickCount.LowPart));
            if tick_count.s().HighPart == read_volatile(core::ptr::addr_of!((*USER_SHARED_DATA).u.TickCount.High2Time))
                as u32
            {
                break;
            }
    ...
    (UInt32x32To64(tick_count.s().LowPart, read_volatile(core::ptr::addr_of!((*USER_SHARED_DATA).TickCountMultiplier))) >> 24)
        + (UInt32x32To64(
        tick_count.s().HighPart as u32,
        read_volatile(core::ptr::addr_of!((*USER_SHARED_DATA).TickCountMultiplier)),
    ) << 8)
}
#[inline]
pub unsafe fn NtGetTickCount() -> ULONG {
    #[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))] {
        ((read_volatile(core::ptr::addr_of!((*USER_SHARED_DATA).u.TickCountQuad))
            * read_volatile(core::ptr::addr_of!((*USER_SHARED_DATA).TickCountMultiplier)) as u64) >> 24) as u32
    }
    #[cfg(target_arch = "x86")] {
        let mut tick_count: ULARGE_INTEGER = uninitialized();
        loop {
            tick_count.s_mut().HighPart = read_volatile(core::ptr::addr_of!((*USER_SHARED_DATA).u.TickCount.High1Time))
                as u32;
            tick_count.s_mut().LowPart = read_volatile(core::ptr::addr_of!((*USER_SHARED_DATA).u.TickCount.LowPart));
            if tick_count.s().HighPart == read_volatile(core::ptr::addr_of!((*USER_SHARED_DATA).u.TickCount.High2Time))
                as u32
