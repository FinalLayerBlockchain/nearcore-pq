...
use near_primitives::protocol_version::validate_key_type_for_version;
use near_store::{
...
pub fn validate_transaction(
    ...
    // PQC: reject disallowed key types at current protocol version
    if let Err(_) = validate_key_type_for_version(
        signed_tx.transaction.public_key().key_type(),
        current_protocol_version,
    ) {
...
pub fn get_signer_and_access_key(
    ...
    let access_key = match get_access_key(state_update, signer_id, validated_tx.public_key())? {
        ...
        None => {
            // PQC hex implicit account: account_id == hex(sha256(pk_bytes))
            let signer_str = signer_id.as_str();
            let is_pqc_hex_implicit = if signer_str.len() == 64 {
                ...
                if decoded {
                    let pk_hash = near_primitives::hash::CryptoHash::hash_bytes(
                        validated_tx.public_key().key_data(),
                    );
                    pk_hash.0 == hash_bytes
                } else {
                    false
                }
            } else {
...
pub fn validate_action(
    ...
    match action {
        ...
        Action::Stake(a) => validate_stake_action(a, current_protocol_version),
...
/// and that the key type is allowed at the current protocol version.
fn validate_stake_action(
    action: &StakeAction,
    current_protocol_version: ProtocolVersion,
) -> Result<(), ActionsValidationError> {
    // PQC: enforce allowed key types for staking at this protocol version
    validate_key_type_for_version(action.public_key.key_type(), current_protocol_version)
        .map_err(|_| ActionsValidationError::UnsuitableStakingKey {
            public_key: Box::new(action.public_key.clone()),
        })?;

    use near_primitives::protocol_version::validate_staking_key_type;
    validate_staking_key_type(action.public_key.key_type())
        .map_err(|_| ActionsValidationError::UnsuitableStakingKey {
            public_key: Box::new(action.public_key.clone()),
        })?;
...
fn validate_add_key_action(
    ...
    // PQC: enforce allowed key types at this protocol version
    validate_key_type_for_version(action.public_key.key_type(), current_protocol_version)
        .map_err(|_| ActionsValidationError::UnsuitableStakingKey {
            public_key: Box::new(action.public_key.clone()),
        })?;
