...
pub struct ValidatorAccountsUpdate {
    ...
    /// Rewards to distribute to validators (added to locked/staked balance).
    pub validator_rewards: HashMap<AccountId, Balance>,
    /// FL: Validator fee rewards (added to liquid/amount balance, not staked).
    /// Computed as validator_claim_fee_bps/10000 of the full epoch reward.
    pub validator_fee_rewards: HashMap<AccountId, Balance>,
    ...
    fn update_validator_accounts(
        ...
        for (account_id, max_of_stakes) in &validator_accounts_update.stake_info {
            if let Some(mut account) = get_account(state_update, account_id)? {
                ...
                // FL: Apply fee rewards as liquid income (not compounded into stake).
                if let Some(fee_reward) = validator_accounts_update.validator_fee_rewards.get(account_id) {
                    if fee_reward.as_yoctonear() > 0 {
                        tracing::debug!(target: "runtime", %account_id, %fee_reward, "account adding fee reward to liquid balance");
                        account.set_amount(account.amount().checked_add(*fee_reward).ok_or_else(|| {
                            RuntimeError::UnexpectedIntegerOverflow("update_validator_accounts fee_reward".into())
                        })?);
                    }
                }
                ...
                if account.locked() < *max_of_stakes {
                    // FL: Staking invariant: locked < max_of_stakes.
                    // This can happen when a staking pool calls stake(0) during initialization
                    // (pool has no depositors yet), intentionally reducing the validator's stake.
                    // In standard NEAR this is a fatal bug, but in FL it is an expected operation.
                    // Resolution: skip return_stake for this validator; locked stays as-is.
                    tracing::warn!(target: "runtime",
                        account_id = %account_id,
                        locked = %account.locked(),
                        max_of_stakes = %max_of_stakes,
                        "FL: validator stake < max_of_stakes (likely staking pool unstaked during init). \
                         Skipping return_stake for this epoch."
                    );
                    set_account(state_update, account_id.clone(), &account);
                    continue;
                }
                    ...
                    .for_each(|(txs, expiration_flags)| {
                        for (tx, non_expired) in txs.iter().zip(expiration_flags) {
                            ...
                            access_keys.entry((signer_id, pubkey)).or_insert_with(|| {
                                let ak_result = get_access_key(&processing_state.state_update, signer_id, pubkey);
                                // PQC hex implicit: account_id == hex(sha256(pk_bytes))
                                if let Ok(None) = &ak_result {
                                    ...
                                    if signer_str.len() == 64 {
                                        ...
                                        if decoded && near_primitives::hash::CryptoHash::hash_bytes(pubkey.key_data()).0 == hash_bytes {
                                            return Ok(Some(near_primitives::account::AccessKey {
                                                nonce: 0,
