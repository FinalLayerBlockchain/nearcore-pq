...
pub fn tx_cost(
    ...
    // PQC signature verify gas constants (from core/primitives/src/runtime/fees.rs)
    const TX_MLDSA_SIGNATURE_VERIFY_GAS:  u64 = 2_100_000_000_000;
    const TX_FNDSA_SIGNATURE_VERIFY_GAS:  u64 = 1_400_000_000_000;
    const TX_SLHDSA_SIGNATURE_VERIFY_GAS: u64 = 3_200_000_000_000;

    let mut cost = calculate_tx_cost(tx.receiver_id(), tx.signer_id(), tx.actions(), config, receipt_gas_price)?;

    // PQC: Charge extra gas for quantum-resistant signature verification.
    // Ed25519 verification cost is already baked into the base transaction fee.
    // PQC signatures are costlier to verify (larger keys/sigs, more compute in WASM).
    let pqc_gas: u64 = match tx.public_key().key_type() {
        KeyType::MLDSA  => TX_MLDSA_SIGNATURE_VERIFY_GAS,
        KeyType::FNDSA  => TX_FNDSA_SIGNATURE_VERIFY_GAS,
        KeyType::SLHDSA => TX_SLHDSA_SIGNATURE_VERIFY_GAS,
        _               => 0,
    };
    if pqc_gas > 0 {
        let surcharge = Gas::from_gas(pqc_gas);
        let surcharge_cost = safe_gas_to_balance(receipt_gas_price, surcharge)?;
        cost.gas_burnt = cost.gas_burnt.checked_add_result(surcharge)?;
        cost.burnt_amount = safe_add_balance(cost.burnt_amount, surcharge_cost)?;
        cost.gas_cost = safe_add_balance(cost.gas_cost, surcharge_cost)?;
        cost.total_cost = safe_add_balance(cost.total_cost, surcharge_cost)?;
    }
