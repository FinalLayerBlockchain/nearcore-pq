    ...
    *actor_id = account_id.clone();
    match account_id.get_account_type() {
        AccountType::NearImplicitAccount => {
            // Final Layer: hex account IDs are SHA-256(PQC_pk), not ED25519 public keys.
            // We do NOT auto-register any implicit key — the user must explicitly add
            // their PQC key via a CreateAccount + AddKey batch transaction.
            // This prevents a useless ED25519 key from being registered on every
            // hex account funded via plain Transfer.
            *account = Some(Account::new(
                deposit,
                Balance::ZERO,
                AccountContract::None,
            ...
            // No set_access_key — account starts with zero access keys.
            let _ = block_height; // suppress unused warning
        }
