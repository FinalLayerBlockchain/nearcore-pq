...
/// Gas: MLDSA_VERIFY_BASE + msg_len * MLDSA_VERIFY_BYTE
pub fn mldsa_verify(
    ctx: &mut Ctx,
    memory: &mut [u8],
    sig_len: u64,
    ...
    use near_crypto::{MlDsaPublicKey, PublicKey, Signature, MLDSA_PUBLIC_KEY_LEN};
    const MLDSA_VERIFY_BASE_GAS: u64 = 2_100_000_000_000;
    const MLDSA_VERIFY_BYTE_GAS: u64 = 5_000_000;
    let total_gas =
        MLDSA_VERIFY_BASE_GAS.saturating_add(msg_len.saturating_mul(MLDSA_VERIFY_BYTE_GAS));
    consume_gas(&mut ctx.result_state.gas_counter, total_gas)?;
    let sig_bytes = read_memory_for_free(memory, sig_ptr, sig_len)?.to_vec();
    let msg_bytes = read_memory_for_free(memory, msg_ptr, msg_len)?;
    let pk_bytes = read_memory_for_free(memory, pk_ptr, MLDSA_PUBLIC_KEY_LEN as u64)?.to_vec();
    let pk = PublicKey::MlDsa(MlDsaPublicKey(pk_bytes));
    let sig = Signature::MlDsa(sig_bytes);
    Ok(sig.verify(msg_bytes, &pk) as u64)
}

/// Verify an FN-DSA (Falcon-512 / FIPS 206) detached signature.
/// Gas: FNDSA_VERIFY_BASE + msg_len * FNDSA_VERIFY_BYTE
pub fn fndsa_verify(
    ctx: &mut Ctx,
    memory: &mut [u8],
    sig_len: u64,
    ...
    use near_crypto::{FnDsaPublicKey, PublicKey, Signature, FNDSA_PUBLIC_KEY_LEN};
    const FNDSA_VERIFY_BASE_GAS: u64 = 1_400_000_000_000;
    const FNDSA_VERIFY_BYTE_GAS: u64 = 5_000_000;
    let total_gas =
        FNDSA_VERIFY_BASE_GAS.saturating_add(msg_len.saturating_mul(FNDSA_VERIFY_BYTE_GAS));
    consume_gas(&mut ctx.result_state.gas_counter, total_gas)?;
    let sig_bytes = read_memory_for_free(memory, sig_ptr, sig_len)?.to_vec();
    let msg_bytes = read_memory_for_free(memory, msg_ptr, msg_len)?;
    let pk_bytes = read_memory_for_free(memory, pk_ptr, FNDSA_PUBLIC_KEY_LEN as u64)?.to_vec();
    let pk = PublicKey::FnDsa(FnDsaPublicKey(pk_bytes));
    let sig = Signature::FnDsa(sig_bytes);
    Ok(sig.verify(msg_bytes, &pk) as u64)
}

/// Verify an SLH-DSA (SPHINCS+-sha2-128s / FIPS 205) detached signature.
/// Gas: SLHDSA_VERIFY_BASE + msg_len * SLHDSA_VERIFY_BYTE
pub fn slhdsa_verify(
    ...
    use near_crypto::{SlhDsaPublicKey, PublicKey, Signature, SLHDSA_PUBLIC_KEY_LEN};
    const SLHDSA_VERIFY_BASE_GAS: u64 = 3_200_000_000_000;
    const SLHDSA_VERIFY_BYTE_GAS: u64 = 5_000_000;
    let total_gas =
        SLHDSA_VERIFY_BASE_GAS.saturating_add(msg_len.saturating_mul(SLHDSA_VERIFY_BYTE_GAS));
    consume_gas(&mut ctx.result_state.gas_counter, total_gas)?;
    let sig_bytes = read_memory_for_free(memory, sig_ptr, sig_len)?.to_vec();
    let msg_bytes = read_memory_for_free(memory, msg_ptr, msg_len)?;
    let pk_bytes = read_memory_for_free(memory, pk_ptr, SLHDSA_PUBLIC_KEY_LEN as u64)?.to_vec();
    let pk = PublicKey::SlhDsa(SlhDsaPublicKey(pk_bytes));
    let sig = Signature::SlhDsa(sig_bytes);
    Ok(sig.verify(msg_bytes, &pk) as u64)
}
