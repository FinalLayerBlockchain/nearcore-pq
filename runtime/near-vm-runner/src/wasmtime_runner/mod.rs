...
impl WasmtimeVM {
    ...
    pub(crate) fn new_for_target(
        ...
        let vm = guard.entry(vm_key).or_insert_with_key(|vm_key| {
            ...
            // Windows does not support signals-based traps, memfd, or disabling native_unwind_info.
            // Only apply Linux/macOS-specific optimizations on non-Windows platforms.
            #[cfg(not(target_os = "windows"))]
            engine_config
                // wasm_backtrace(false) internally disables native_unwind_info, which is forbidden on Windows.
                .wasm_backtrace(false)
                .wasm_backtrace_details(WasmBacktraceDetails::Disable)
                // Enable copy-on-write heap images (Linux/macOS only).
            ...
            // Windows cannot support the pooling allocator's virtual-memory reservation
            // (1000 instances × ~134 MB = ~134 GB of VA space).  Fall back to on-demand
            // allocation on Windows so Engine::new always succeeds.
            #[cfg(target_os = "windows")]
            {
                engine_config
                    ...
                    .wasm_backtrace_details(WasmBacktraceDetails::Disable)
                    .memory_guard_size(0)
                    .memory_reservation(max_memory_size.try_into().unwrap_or(64 * 1024 * 1024))
                    .memory_reservation_for_growth(0);
            }
