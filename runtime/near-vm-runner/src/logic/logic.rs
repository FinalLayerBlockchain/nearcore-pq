...
bls12381_p2_decompress_base + bls12381_p2_decompress_element * num_elements`
    ...
    /// Available only when `pqc_host_fns` config flag is enabled (protocol >= 1000).
    pub fn mldsa_verify(
        &mut self,
        sig_len: u64,
        sig_ptr: u64,
        ...
        let total_gas = MLDSA_VERIFY_BASE_GAS
            .saturating_add(msg_len.saturating_mul(MLDSA_VERIFY_BYTE_GAS));
        self.burn_gas(total_gas)?;
        let sig_bytes = get_memory_or_register!(self, sig_ptr, sig_len)?;
        let msg_bytes = get_memory_or_register!(self, msg_ptr, msg_len)?;
        let pk_bytes = get_memory_or_register!(self, pk_ptr, MLDSA_PUBLIC_KEY_LEN as u64)?;
        let pk = PublicKey::MlDsa(MlDsaPublicKey(pk_bytes.to_vec()));
        let sig = Signature::MlDsa(sig_bytes.to_vec());
    ...
    pub fn fndsa_verify(
        ...
        let total_gas = FNDSA_VERIFY_BASE_GAS
            .saturating_add(msg_len.saturating_mul(FNDSA_VERIFY_BYTE_GAS));
        self.burn_gas(total_gas)?;
        let sig_bytes = get_memory_or_register!(self, sig_ptr, sig_len)?;
        let msg_bytes = get_memory_or_register!(self, msg_ptr, msg_len)?;
        let pk_bytes = get_memory_or_register!(self, pk_ptr, FNDSA_PUBLIC_KEY_LEN as u64)?;
        let pk = PublicKey::FnDsa(FnDsaPublicKey(pk_bytes.to_vec()));
        let sig = Signature::FnDsa(sig_bytes.to_vec());
        Ok(sig.verify(&msg_bytes, &pk) as u64)
    }
    ...
    pub fn slhdsa_verify(
        ...
        let total_gas = SLHDSA_VERIFY_BASE_GAS
            .saturating_add(msg_len.saturating_mul(SLHDSA_VERIFY_BYTE_GAS));
        self.burn_gas(total_gas)?;
        let sig_bytes = get_memory_or_register!(self, sig_ptr, sig_len)?;
        let msg_bytes = get_memory_or_register!(self, msg_ptr, msg_len)?;
        let pk_bytes = get_memory_or_register!(self, pk_ptr, SLHDSA_PUBLIC_KEY_LEN as u64)?;
        let pk = PublicKey::SlhDsa(SlhDsaPublicKey(pk_bytes.to_vec()));
        let sig = Signature::SlhDsa(sig_bytes.to_vec());
        Ok(sig.verify(&msg_bytes, &pk) as u64)
    }
