...
impl From<near_crypto::KeyType> for CurveType {
    fn from(key_type: near_crypto::KeyType) -> Self {
        match key_type {
            ...
            // PQC key types are not supported in Rosetta RPC
            near_crypto::KeyType::MLDSA
            | near_crypto::KeyType::FNDSA
            | near_crypto::KeyType::SLHDSA => Self::Edwards25519,
        }
    }
}
...
impl TryFrom<near_crypto::KeyType> for SignatureType {
    ...
    fn try_from(key_type: near_crypto::KeyType) -> Result<Self, Self::Error> {
        match key_type {
            ...
            near_crypto::KeyType::MLDSA
            | near_crypto::KeyType::FNDSA
            | near_crypto::KeyType::SLHDSA => Err(crate::errors::ErrorKind::InvalidInput(
                "PQC keys are not supported in Rosetta RPC".to_string(),
            )),
        }
    }
}
