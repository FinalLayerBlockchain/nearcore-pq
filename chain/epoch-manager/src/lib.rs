...
impl EpochManager {
    ...
    /// Final Layer halving schedule for staking APY.
    ///
    /// The schedule halvings the max inflation rate every 4 years, starting at 10%,
    /// with a perpetual floor of 0.5% after year 20. Designed to last 100+ years
    /// while maintaining reasonable validator incentives.
    ///
    /// | Era | Epochs      | Years | APY    |
    /// |-----|-------------|-------|--------|
    /// |  0  | 0 – 2919    | 1–4   | 10%    |
    /// |  1  | 2920–5839   | 5–8   | 5%     |
    /// |  2  | 5840–8759   | 9–12  | 2.5%   |
    /// |  3  | 8760–11679  | 13–16 | 1.25%  |
    /// |  4  | 11680–14599 | 17–20 | 0.625% |
    /// | 5+  | 14600+      | 21+   | 0.5%   |  ← perpetual floor
    ///
    /// ~730 epochs/year (43 200-block epochs at 1 block/sec).
    fn fl_inflation_rate_for_epoch(epoch_height: u64) -> Rational32 {
        const EPOCHS_PER_HALVING: u64 = 2920; // 4 years × ~730 epochs/year
        let halvings = epoch_height / EPOCHS_PER_HALVING;
        if halvings >= 5 {
            // Perpetual floor: 0.5% = 1/200
            Rational32::new(1, 200)
        } else {
            // 10% halving each era: 1/10, 1/20, 1/40, 1/80, 1/160
            Rational32::new(1, 10i32 << halvings)
        }
    }
    ...
    fn finalize_epoch(
        ...
        let (validator_reward, minted_amount) = {
            ...
            // v1002: pass epoch_height so calculate_reward can determine APY year.
            // fl_inflation_rate_for_epoch is unused in v1002 but kept for history.
            let dummy_inflation = Rational32::new(0, 1);
