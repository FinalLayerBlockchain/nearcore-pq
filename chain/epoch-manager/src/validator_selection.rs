...
use near_chain_configs::FL_MIN_BLOCK_PRODUCER_STAKE;
...
/// Final Layer v1002: proposals with stake below FL_MIN_BLOCK_PRODUCER_STAKE (1 000 FLC)
/// are filtered out before the auction.  This prevents dust proposals from polluting
/// the validator set and ensures every active validator has meaningful skin in the game.
fn select_validators_from_proposals(
    ...
    // Filter out proposals below the FL minimum stake before running the auction.
    let proposals: HashMap<AccountId, ValidatorStake> = proposals
        .into_iter()
        .filter(|(_, vs)| vs.stake() >= FL_MIN_BLOCK_PRODUCER_STAKE)
        .collect();
