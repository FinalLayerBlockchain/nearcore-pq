...
impl SpiceCoreReader {
    ...
    pub fn get_last_certified_execution_results_for_next_block(
        ...
        // First spice activation: if the last certified block was in a pre-spice epoch,
        // no execution results were written. Build from ChunkExtras instead.
        let last_cert_protocol = self
            .epoch_manager
            .get_epoch_protocol_version(last_certified_block_header.epoch_id())?;
        if !ProtocolFeature::Spice.enabled(last_cert_protocol) {
            let shard_layout =
                self.epoch_manager.get_shard_layout(last_certified_block_header.epoch_id())?;
            let mut results = HashMap::new();
            for shard_id in shard_layout.shard_ids() {
                let cid = SpiceChunkId { block_hash: last_certified_hash, shard_id };
                if let Ok(chunk_extra) = self.get_trusted_chunk_extra(&cid) {
                    results.insert(shard_id, Arc::new(ChunkExecutionResult {
                        chunk_extra: Arc::unwrap_or_clone(chunk_extra),
                        outgoing_receipts_root: CryptoHash::default(),
                    }));
                }
            }
            return Ok(BlockExecutionResults(results));
        }
        ...
        // Fallback: if walk-back didn't find all results (e.g., after node restart),
        // read from persisted DBCol::execution_results store.
        if execution_results.len() < num_shards {
            if let Ok(shard_ids) = self.epoch_manager.shard_ids(last_certified_block_header.epoch_id()) {
                for shard_id in shard_ids {
                    if !execution_results.contains_key(&shard_id) {
                        if let Some(result) = self.get_execution_result_from_store(&last_certified_hash, shard_id) {
                            execution_results.insert(shard_id, result);
                        }
                    }
                }
            }
