...
pub(crate) struct ThreadPool {
    ...
    name: &'static str,
    /// Priority of spawned threads (must be in [0; 100] range, Unix only)
    #[cfg(not(windows))]
...
impl ThreadPool {
    ...
    pub(crate) fn new(
        ...
        #[cfg(not(windows))]
        let priority: ThreadPriority = priority.try_into().expect("priority out of range");
        #[cfg(windows)]
        let _ = priority; // unused on Windows
        Self {
            name,
            #[cfg(not(windows))]
            priority,
...
mod tests {
    ...
    #[test]
    #[cfg_attr(not(windows), should_panic(expected = "priority out of range"))]
    fn invalid_priority() {
