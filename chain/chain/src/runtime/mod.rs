    ...
    fn process_state_update(
        ...
        let validator_accounts_update = {
            ...
            if epoch_manager.is_next_block_epoch_start(prev_block_hash)? {
                let (stake_info, full_validator_reward) =
                ...
                // FL: Split full reward into net stake reward + liquid fee reward.
                // validator_claim_fee_bps is in basis points: 100 bps = 1%, capped at 1000 (10%).
                // Net reward compounds into locked stake; fee reward goes to liquid balance.
                let mut validator_rewards: HashMap<AccountId, Balance> = HashMap::new();
                let mut validator_fee_rewards: HashMap<AccountId, Balance> = HashMap::new();
                for (account_id, full_reward) in full_validator_reward {
                    if shard_layout.account_id_to_shard_id(&account_id) != shard_id {
                        continue;
                    }
                    let fee_bps = (self.genesis_config
                        .validator_claim_fee_bps
                        .get(&account_id)
                        .copied()
                        .unwrap_or(0)
                        .min(1000)) as u128;
                    if fee_bps > 0 {
                        let fee_reward = full_reward.as_yoctonear() * fee_bps / 10_000;
                        let net_reward = full_reward.as_yoctonear().saturating_sub(fee_reward);
                        validator_rewards.insert(account_id.clone(), Balance::from_yoctonear(net_reward));
                        validator_fee_rewards.insert(account_id, Balance::from_yoctonear(fee_reward));
                    } else {
                        validator_rewards.insert(account_id, full_reward);
                    }
                }
