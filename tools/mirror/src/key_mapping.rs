...
/// Derives an ED25519SecretKey from arbitrary key bytes, using a SHA-256 hash
/// as the 32-byte representative seed so the derivation is deterministic.
fn map_pqc_key(pub_bytes: &[u8], secret: Option<&[u8; crate::secret::SECRET_LEN]>) -> SecretKey {
    // SHA-256 hash of the PQC public key gives us a stable 32-byte representative.
    let hash: [u8; 32] = Sha256::digest(pub_bytes).into();
    let representative = ED25519PublicKey(hash);
    SecretKey::ED25519(map_ed25519(&representative, secret))
}
...
pub fn map_key(key: &PublicKey, secret: Option<&[u8; crate::secret::SECRET_LEN]>) -> SecretKey {
    match key {
        ...
        // PQC keys: hash the public key to a 32-byte seed and derive an ED25519 key.
        // This is deterministic and avoids panics; PQC source chains are not expected
        // in the mirror tool, but we handle them gracefully.
        PublicKey::MlDsa(k) => map_pqc_key(&k.0, secret),
        PublicKey::FnDsa(k) => map_pqc_key(&k.0, secret),
        PublicKey::SlhDsa(k) => map_pqc_key(&k.0, secret),
    }
}
