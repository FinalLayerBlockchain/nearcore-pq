...
// jemalloc disabled (not supported on Windows MSVC)
...
fn main() -> anyhow::Result<()> {
    ...
    #[cfg(not(windows))]
    {
        ...
        let (soft, hard) =
            rlimit::Resource::NOFILE.get().context("rlimit::Resource::NOFILE::get()")?;
    ...
    // Windows: no NOFILE rlimit — OS manages FD limits automatically.
