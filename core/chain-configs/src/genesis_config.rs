...
pub struct GenesisConfig {
    ...
    /// FL: Per-validator claim fee in basis points (100 bps = 1%, max 1000 = 10%).
    /// This fraction of each epoch's staking reward is paid to the validator as liquid
    /// income; the remainder compounds back into the validator's locked stake.
    /// Example: {"validator-1.fl": 500, "validator-2.fl": 300} -> 5% and 3% fees.
    /// Absent or 0 = no fee. Values above 1000 are clamped to 1000 (10% max).
    #[serde(default)]
    pub validator_claim_fee_bps: std::collections::HashMap<near_primitives::types::AccountId, u32>,
    /// FL: Per-validator deposit fee in basis points (max 10 = 0.1%).
    /// Charged once when a delegator stakes to this validator (deducted from the deposit).
    /// Values above 10 are clamped to 10.
    /// Note: deposit fees require staking pool delegation support to take effect.
    #[serde(default)]
    pub validator_deposit_fee_bps: std::collections::HashMap<near_primitives::types::AccountId, u32>,
}
