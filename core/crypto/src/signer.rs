use crate::key_conversion::{convert_secret_key, pqc_derive_vrf_secret};
    ...
    pub fn compute_vrf_with_proof(&self, data: &[u8]) -> (crate::vrf::Value, crate::vrf::Proof) {
        match &self.secret_key {
            SecretKey::ED25519(sk) => {
                convert_secret_key(sk).compute_vrf_with_proof(&data)
            }
            SecretKey::MlDsa(sk) => {
                pqc_derive_vrf_secret(&sk.0[..crate::signature::MLDSA_PUBLIC_KEY_LEN])
                    .compute_vrf_with_proof(&data)
            }
            SecretKey::FnDsa(sk) => {
                pqc_derive_vrf_secret(&sk.0[..crate::signature::FNDSA_PUBLIC_KEY_LEN])
                    .compute_vrf_with_proof(&data)
            }
            SecretKey::SlhDsa(sk) => {
                pqc_derive_vrf_secret(&sk.0[..crate::signature::SLHDSA_PUBLIC_KEY_LEN])
                    .compute_vrf_with_proof(&data)
            }
            SecretKey::SECP256K1(_) => {
                panic!("SECP256K1 keys cannot be used for VRF computation")
            }
        }
    }
