...
impl NoFile for RealNoFile {
    fn get(&self) -> std::io::Result<(u64, u64)> {
        #[cfg(not(windows))]
        return rlimit::Resource::NOFILE.get();
        #[cfg(windows)]
        return Ok((u64::MAX, u64::MAX));  // Windows has no NOFILE rlimit concept
    }

    fn set(&mut self, _soft: u64, _hard: u64) -> std::io::Result<()> {
        #[cfg(not(windows))]
        return rlimit::Resource::NOFILE.set(_soft, _hard);
        #[cfg(windows)]
