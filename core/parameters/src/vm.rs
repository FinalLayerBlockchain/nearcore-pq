...
pub struct Config {
    ...
    /// Whether to enable PQC signature verify host functions (mldsa_verify, fndsa_verify, slhdsa_verify).
    pub pqc_host_fns: bool,

    /// Whether ed25519_verify is available in the WASM import table (disabled on PQC chain).
